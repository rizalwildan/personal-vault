...
- [ ] All endpoints handle authorization errors correctly (401 or custom 403)
...
- [ ] Task 1: Implement GET /api/v1/notes/:id in NotesService (AC: 1, 2, 3)
  ...
  - [ ] Call repository to fetch note: `repository.findByUserAndId(userId, noteId)`
  - [ ] If note not found, throw NotFoundError('Note')
  - [ ] Return note object (user_id should match for verification)
  - [ ] Verify user_id matches authenticated userId (double-check for security)
  - [ ] If mismatch, throw UnauthorizedError('Access denied')
...
- [ ] Task 2: Implement GET /api/v1/notes/:id Route Handler (AC: 1, 2, 3, 19)
  ...
  - [ ] Error handling: NotFoundError → 404, UnauthorizedError → 401

- [ ] Task 3: Implement Repository Methods for Update/Delete (AC: 5, 6, 7)
  - [ ] Add to NotesRepository:
    - [ ] `findByUserAndId(userId, noteId)` - Find with ownership check
    - [ ] `update(noteId, updates)` - Update fields, return updated note
    - [ ] `softDelete(noteId)` - Set is_archived=true
  - [ ] `findByUserAndId()` method:
...
- [ ] Task 4: Implement PATCH /api/v1/notes/:id in NotesService (AC: 5, 6, 7, 8, 9, 10, 11)
  ...
  - [ ] If content changed:
    ...
    - [ ] Use combined text: `${updates.title || currentNote.title}\n\n${updates.content || currentNote.content}`
